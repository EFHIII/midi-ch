    ...
    if(settings.tracks[i]) {
      for(let note = 0; note < currentMidi.tracks[i].notes.length; note++) {
        ...
        unChartedNotes.push([
          ...
          note, //10
          [
            document.getElementById(`midi${i}-${currentMidi.tracks[i].notes[note].midi}-1`).checked,
            document.getElementById(`midi${i}-${currentMidi.tracks[i].notes[note].midi}-2`).checked,
            document.getElementById(`midi${i}-${currentMidi.tracks[i].notes[note].midi}-3`).checked,
            document.getElementById(`midi${i}-${currentMidi.tracks[i].notes[note].midi}-4`).checked,
            document.getElementById(`midi${i}-${currentMidi.tracks[i].notes[note].midi}-5`).checked,
            document.getElementById(`midi${i}-${currentMidi.tracks[i].notes[note].midi}-6`).checked,
            document.getElementById(`midi${i}-${currentMidi.tracks[i].notes[note].midi}-7`).checked,
            document.getElementById(`midi${i}-${currentMidi.tracks[i].notes[note].midi}-8`).checked,
...
function drawNote(cn, y) {
  ...
  if(cymbal) {
    ...
    arc(note * width / 5 + width / 5, y, width / 8, width / 8, Math.PI * 7 / 6, Math.PI * 11 / 6);
  }
}
    ...
    if(Y > 0 && Y < 0.9 * height) {
      ...
      while(myLastNote > 0 && unChartedNotes[myLastNote][0] == lastT) {
        if(note > 0 && unChartedNotes[myLastNote][1] == currentNote[1] && chartedNotes[note] != chartedNotes[myLastNote]) {
          fill(255, 0, 0, 150);
          //rect(chartedNotes[note] * width / 5 + width / 5 - 10, Y, 20, (currentNote[3] - preview.time) / preview.scale * height - (unChartedNotes[myLastNote][3] - preview.time) / preview.scale * height);
