...
function loadSettings(){
    ...
    if(settings.tracks[i]){
      for(var note=0;note<currentMidi.tracks[i].notes.length;note++){
        unChartedNotes.push([
          ...
          currentMidi.tracks[i].notes[note].durationTicks<=stripSustain?0:currentMidi.tracks[i].notes[note].durationTicks,//2
  ...
  for(var i=0;i<groups.length;i++){
    if(findInGroups(groups[i][groups[i].length-1]+1)[0]>=0){
      ...
      var b=groups[findInGroups(groups[i][groups[i].length-1]+1)[0]][0];
...
function loadHTMLcontent(){
  ...
  <button class="btn btn-primary" style="width:100%" onclick="loadSettings()"><span data-toggle="tooltip" title="Re-charts the song based on the new settings and any recently deleted notes">Load New Settings</button><br>
    ...
    <div class="custom-control">
      <input type="number" id="stripSustain" value=0 min="0">
...
drawNotesPreview.prototype.draw=function() {
  if(preview.time/currentMidi.duration*this.w<this.lastTime){this.at=0;this.lastTime=0;}
